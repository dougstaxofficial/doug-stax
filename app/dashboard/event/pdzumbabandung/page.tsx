...
export default function PowerDuoZumbaBandungPage() {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          user_id: user.id,
          event_name: 'Power Duo Zumba Bandung',
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-black bg-gradient-to-r from-purple-600 to-fuchsia-600 bg-clip-text text-transparent">
                  Power Duo Zumba Bandung
                </h1>
                <p className="text-xs font-semibold text-gray-600">15 Maret 2026 • GOR Saparua</p>
              </div>
            </div>
            <Link 
